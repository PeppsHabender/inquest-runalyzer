...
class ChartModel {
  ...
  (double, double, double, double) minXMaxXminYMaxY() {
    final Iterable<LineChartBarData> unzipped = bars
        .where((e) => e.id != RunalyzerLineChart.HIDDEN_ID)
        .map((el) => el.data);
    final (double, double) minMaxX =
        _combineMinMax(unzipped.map((el) => _findMinMax(el.spots, (s) => s.x)));
    final (double, double) minMaxY = this.minMaxY ??
        _combineMinMax(unzipped.map((el) => _findMinMax(el.spots, (s) => s.y)));
...
class LineChartBar {
  ...
  LineChartBar(this.id, this.data, {this.tooltips, this.drawAverage = true}) {
    ...
    averageY =
        data.spots.map((e) => e.y).reduce((a, b) => a + b) / data.spots.length;
  }
}

(double, double) _combineMinMax(Iterable<(double, double)> ls) =>
    ls.reduce((acc, curr) => (min(acc.$1, curr.$1), max(acc.$2, curr.$2)));

(double, double) _findMinMax(
        Iterable<FlSpot> ls, double Function(FlSpot) extractor) =>
    ls.fold((double.infinity, double.negativeInfinity), (minMax, curr) {
