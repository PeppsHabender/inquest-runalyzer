...
class StaticModifierController extends GetxController {
  ...
  StaticModifierController(this._static) {
    ...
    players.value = _static.players.map((e) {
      final Commander? commander = _static.commanders.firstWhereOrNull((el) => e == el.accountName);

      return commander == null ? CommanderModel.fromString(e) : CommanderModel.fromCommander(commander);
    }).toList();
    players.sort();
  ...
  void updateStatic() async {
    ...
    await doHttpRequest(
      ...
      body: <String, dynamic>{
        "name"       : name.value,
        "day"        : dayOfWeek.text,
        "time"       : "${hour.value.toString().padLeft(2, "0")}:${minute.value.toString().padLeft(2, "0")}",
        "type"       : type.text,
...
class CommanderModel with Compare<CommanderModel> {
  ...
  int compareTo(final CommanderModel other) {
    int compare = canModify.value.not().toInt() - other.canModify.value.not().toInt();
    if(compare != 0) return compare;

    compare = canUpload.value.not().toInt() - other.canUpload.value.not().toInt();
    if(compare != 0) return compare;
