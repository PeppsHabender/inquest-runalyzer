...
class StaticEditor extends StatelessWidget {
  ...
  Widget _metaEditor() => Row(
        children: [
          ...
          DropdownMenu<StaticType>(
              ...
              initialSelection: StaticType.values
                  .firstWhere((e) => _controller.type.text == e.name),
              dropdownMenuEntries: StaticType.values
  ...
  Widget _timeSlotEditor() => Row(
        children: [
          DropdownMenu<DayOfWeek>(
              ...
              initialSelection: DayOfWeek.values
                  .firstWhere((e) => _controller.dayOfWeek.text == e.name),
          ...
          SizedBox(
            ...
            child: RxTextField(
              ...
              validator: (h) =>
                  int.tryParse(h)?.inRange(min: 0, max: 23) ?? false,
              maxChars: 2,
            ),
          ),
          const Padding(
          ...
          SizedBox(
            ...
            child: RxTextField(
              ...
              validator: (m) =>
                  int.tryParse(m)?.inRange(min: 0, max: 59) ?? false,
              maxChars: 2,
            ),
          )
        ],
  ...
  Widget _playersEditor(final BuildContext context) => Stack(children: [
        ObxValue(
            (players) => ListView.builder(
                controller: _playerScrollController,
                itemCount: players.length,
                itemBuilder: (context, idx) => _playerEditor(
                    players[idx], _controller.creator, players.remove)),
            _controller.players),
        Padding(
          ...
          child: Align(
            ...
            child: IconButton(
              ...
              onPressed: () {
                _controller.players
                    .add(CommanderModel.fromString("Player.1234"));
                Future.delayed(
                    const Duration(milliseconds: 200),
                    () => _playerScrollController.scrollDown(
  ...
  Widget _updater() => Row(mainAxisAlignment: MainAxisAlignment.end, children: [
        ObxValue(
        ...
        const SizedBox(width: 10),
        Obx(() => OutlinedButton(
            onPressed: _controller.loading.value || _controller.hasErrors()
                ? null
                : _controller.updateStatic,
            child: const Text("Update"))),
      ]);
}

Widget _playerEditor(final CommanderModel player, final String creator,
    final void Function(CommanderModel) remove) {
  final RxBool hovered = false.obs;
  final RxBool tapped = false.obs;

  return InkWell(
    onHover: (h) => hovered.value = h,
    onTap: player.name.value == creator
        ? null
        : () {
    ...
    child: ListTile(
      ...
      title: Obx(() => tapped.value
          ...
          : Text(player.name.text,
              style: TextStyle(
                  color: player.name.text == creator ? Colors.white38 : null))),
      trailing: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _checkBox(player.name.text != creator, player.canUpload,
              const FeatherIcon(FeatherIcons.upload, size: 25)),
          const SizedBox(width: 10),
          _checkBox(
              player.name.text != creator,
              player.canModify,
              Image(
                  width: 25,
                  height: 52,
                  image: RunalyzerAssets.randomCommander()))
        ],
      ),
    ),
  );
...
Widget _playerTextField(final RxBool tapped, final TextEditingController name,
...
Widget _checkBox(final bool enabled, final RxBool prop, final Widget icon) =>
    Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        ObxValue(
