...
class RunalyzerDataTable extends StatelessWidget {
  ...
  RunalyzerDataTable({super.key, required this.columns, required this.rows});
  ...
  DataColumn _dataColumn(final int idx, final RunalyzerColumn column) =>
      DataColumn(
        ...
        onTap: () {
          ...
          _sortedIdx.value =
              (idx: idx, ascending: oldIdx == idx ? !oldAsc : false);
        },
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: <Widget>[
            Text(column.title, style: const TextStyle(fontSize: 16)),
            _sortedIdx.value?.idx != idx
                ? const SizedBox(width: 20)
                : Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children:
                        _sortedIdx.value == null || _sortedIdx.value!.idx != idx
                            ? []
                            : [
                                _sortIcon(Icons.arrow_drop_up_sharp, 5,
                                    _sortedIdx.value!.ascending),
                                _sortIcon(Icons.arrow_drop_down_sharp, -5,
                                    !_sortedIdx.value!.ascending)
                              ]),
          ],
        ),
  ...
  Widget _sortIcon(
          final IconData icon, final double yOffs, final bool selected) =>
      Transform.translate(
        offset: Offset(0, yOffs),
        child: Icon(icon,
            color: selected ? Colors.white : Colors.white38, size: 20),
      );
  ...
  List<RunalyzerRow> _sortRows(final _Sort sort) {
    ...
    final List<RunalyzerRow> sorted = rows
      ..sort((a, b) => comparator(a.cells[idx].data, b.cells[idx].data));
