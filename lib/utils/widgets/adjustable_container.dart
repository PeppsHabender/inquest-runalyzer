...
class HeightAdjustableContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    ...
    children: [
      ObxValue((h) => SizedBox(height: max(h.value, minHeight), child: child), height),
      GestureDetector(
        onVerticalDragEnd: (details) => initHeight.value = height.value,
        onVerticalDragUpdate: (details) => height.value = initHeight.value + details.localPosition.dy,
        child: MouseRegion(
          ...
          child: Row(
            children: [
              ...
              const MouseRegion(cursor: SystemMouseCursors.click, child: Icon(Icons.height_outlined, size: 25)),
              Expanded(child: Padding(
