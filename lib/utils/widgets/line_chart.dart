...
class RunalyzerLineChart extends StatelessWidget {
  ...
  RunalyzerLineChart(
    this._model,
    {
      List<VerticalLine> Function(double, double)? verticalLines,
      super.key
    }
        ...
        return Column(
          children: [
            Row(
              children: [
                Expanded(
                  child: _selectionBoxes(_model, unselected)
                ),
            ...
            Expanded(
              child: _theLineChart(context, _model, unselected, _verticalLines)
            ),
          ],
        );
      },
...
Widget _selectionBoxes(final ChartModel model, final Set<String> unselected) => Wrap(
  direction: Axis.horizontal,
  children: model.bars.where((e) => e.id != RunalyzerLineChart.HIDDEN_ID).map((e) => Row(
    mainAxisSize: MainAxisSize.min,
    children: [
      Checkbox(
        value: !unselected.contains(e.id),
        onChanged: (_) => unselected.contains(e.id) ? unselected.remove(e.id) : unselected.add(e.id),
        activeColor: e.data.color,
        side: BorderSide(color: e.data.color ?? Colors.white, width: 1.5),
      ),
      Text(e.id),
...
Widget _selectAllButtons(final ChartModel model, final Set<String> unselected) => Row(
  children: [
    IconButton(
...
Widget _theLineChart(
  ...
  return LineChart(
    LineChartData(
      ...
      lineBarsData: model.bars.where((e) => !unselected.contains(e.id)).map((el) => el.data).toList(),
      extraLinesData: ExtraLinesData(
        extraLinesOnTop: false,
        horizontalLines: model.bars.where((e) => e.drawAverage && !unselected.contains(e.id)).where((e) => e.averageY != null).map((e) => HorizontalLine(
          y: e.averageY!,
          strokeWidth: 1,
          dashArray: [4, 4],
          color: e.data.color?.withAlpha(160)
        )).toList(),
        verticalLines: verticalLines?.call(minMaxXMinMaxY.$1, minMaxXMinMaxY.$2) ?? []
      ),
      ...
      gridData: FlGridData(show: true, verticalInterval: model.bottomTitle.interval, horizontalInterval: model.leftTitle.interval)
    ),
...
FlTitlesData _titles(final ChartModel model) => FlTitlesData(
  ...
  bottomTitles: AxisTitles(
    sideTitles: model.bottomTitle.copyWith(
      getTitlesWidget: (x, meta) => x == meta.min || x  == meta.max ? const Text("") : model.bottomTitle.getTitlesWidget(x, meta)
    )
  ),
  leftTitles: AxisTitles(sideTitles: model.leftTitle),
);

LineTouchData _touchData(final BuildContext context, final ChartModel model, final Set<String> unselected) => LineTouchData(
  touchTooltipData: LineTouchTooltipData(
    ...
    getTooltipItems: (ls) {
      return ls.indexed.map((e) {
        final LineChartBar bar = model.bars.where((e) => !unselected.contains(e.id)).toList()[e.$2.barIndex];
        final String? tooltip = bar.tooltips?[e.$2.spotIndex];

        return LineTooltipItem(
          tooltip ?? e.$2.x.toString(),
          TextStyle(
            color: bar.id == RunalyzerLineChart.HIDDEN_ID ? Colors.white : e.$2.bar.gradient?.colors.first ?? e.$2.bar.color ?? Colors.blueGrey,
            fontWeight: bar.id == RunalyzerLineChart.HIDDEN_ID ? FontWeight.bold : null,
